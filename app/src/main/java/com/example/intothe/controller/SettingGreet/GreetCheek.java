...
import com.example.intothe.databinding.GreetCheekBinding;

import java.util.Random;

public class GreetCheek extends AppCompatActivity {

    private GreetCheekBinding binding;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = GreetCheekBinding.inflate(getLayoutInflater());
        setContentView(binding.getRoot());

        String stGreeting = "";

        // db start
        UserDBHelper myDb = new UserDBHelper(GreetCheek.this);
        SQLiteDatabase db = myDb.getReadableDatabase();
        ReportDBHelper myDb2 = new ReportDBHelper(GreetCheek.this);
